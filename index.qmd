...
colors <- tibble(language = c("Guro", "Dan Gweetaa", "Dan_Gweetaa", "Mano", "Kpelle", "Kono", "Looma", "Bamana"),
       color = c("gold3", "gold1", "gold1","gold1", "cyan1", "cyan1", "cyan3", "dodgerblue4"))
```
...
Всего `r unique(df_pairwise_total$total)` стимула в каждом языке:
...
Всего стимулов 130, но некоторые из них не попали в датасет: `r (1:130)[!(1:130 %in% unique(df$number))]`
...
Вот список стимулов, для которых есть данные всех семи языков (всего таких случаев `r length(to_keep)`):
...
df |> 
  ...
  mutate(construction_type_new = case_when(construction_type_new %in% c("X Aux Y V" ,
                                           ...
                                           construction_type_new == "X Aux V loc" ~ "NOM_loc",
                                           !is.na(adposition_etymology) & str_detect(construction_type, "^X ") ~ str_c("NOM_", adposition_etymology),
                                           !is.na(adposition_etymology) & str_detect(construction_type, "^Y ") ~ str_c(adposition_etymology, "_NOM"),
                                           TRUE ~ construction_type_new)) |> 
  select(number, language, construction, construction_type, construction_type_new) |> 
