...
Вот список стимулов, для которых есть данные всех семи языков:
...
```{r etymology-hh}
et <- readxl::read_xlsx("../GM_merged_wide_verb_MK_0908.xlsx")
et |> 
...
```{r ad-etymology-hh}
et <- readxl::read_xlsx("../GM_merged_wide_adpositions_MK_0807.xlsx")
et |> 
  select(number, predicate_eng, stimuli, ends_with("etym")) |> 
  pivot_longer(names_to = "source", 
               values_to = "value", 
...
```{r}
complex_verbs <- readxl::read_xlsx("../GM_merged_wide_verb_meaning_MK_0807.xlsx")
...
complex_verbs |> 
  ...
  separate_wider_delim(verb_meaning, delim = " ",
                       ...
                       too_few = "align_start") |> 
  filter(!is.na(p2)) |> 
  mutate(n_parts = ifelse(is.na(p3), 2, 3)) |> 
  pivot_longer(names_to = "p", values_to = "form", p1:p3) |> 
  mutate(form = case_when(str_detect(form, "\\?{1,3}") ~ NA,
                          form %in% c("prev", "prev2", "PREF2") ~ NA,
                          TRUE ~ form)) |> 
  na.omit() |> 
  arrange(form, number) |> 
...
map(sort(unique(analyze_df$number)), function(i){
  analyze_df |> 
    ...
    mutate(total = ifelse(str_detect(item1, "3") | str_detect(item2, "3"), 3, 2),
           value = n/total,
           number = i,
           item1 = str_remove(item1, "_\\d"),
           item2 = str_remove(item2, "_\\d")) |> 
    select(number, item1, item2, value) |> 
    filter(item1 != item2)
}) |> 
  list_rbind() ->
  result
