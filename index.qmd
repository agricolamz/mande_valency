...
df_pairwise_total %>% 
  ...
  mutate(percantage = round(n/total*100, 3)) %>% 
  select(item1, item2, percantage) %>% 
  pivot_wider(names_from = item2, values_from = percantage) %>% 
  arrange(item1) %>% 
  select(item1, Dan_Gweetaa, Guro, Kono, Kpelle, Looma, Mano)
```
...
df_pairwise_total %>% 
  left_join(df_pairwise_within_construction) %>% 
  mutate(percantage = round(n/total*100, 2),
  ...
  geom_tile(aes(fill = percantage), colour = "white") +
  geom_text(aes(label = str_c(percantage, "%")), colour = "white") +
...
read_csv("../GM_merged.csv") %>% 
  ...
  mutate(id = 1:n()) %>%
  ungroup() %>% 
  select(number, id, n, extended_construction, Guro, Looma, Mano, Dan_Gweetaa, Kono, Kpelle) %>% 
...
read_csv("../GM_merged.csv") %>% 
  ...
  mutate(id = 1:n()) %>%
  ungroup() %>% 
  select(number, id, extended_construction, Guro, Looma, Mano, Dan_Gweetaa, Kono, Kpelle) %>% 
