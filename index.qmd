...
colors <- tibble(language = c("Guro", "Dan Gweetaa", "Mano", 
                    "Kpelle", "Kono", "Looma", "Dan_Gweetaa"),
       color = c(rep(c("#EEAD0E", "cyan3"), each = 3), "#EEAD0E"))
```
...
df %>% 
  ...
  slice(1)  %>% 
  ungroup() %>% 
  mutate(construction_type = 1) %>% 
  pivot_wider(names_from = language, values_from = construction_type, values_fill = 0) %>% 
...
sample_random_construction %>% 
  hclust() %>% 
  as.phylo() %>% 
  plot(main = "Clusterization with a random equivalent",
       tip.color = colors$color[match(.$tip.label, colors$language)],
       cex = 2)
```
...
stimuli_sample_all_langs_and_random %>% 
  hclust() %>% 
  as.phylo()  %>% 
  plot(main = "Clusterization with a random equivalent (only common stimuli)",
       tip.color = colors$color[match(.$tip.label, colors$language)],
       cex = 2)
```
